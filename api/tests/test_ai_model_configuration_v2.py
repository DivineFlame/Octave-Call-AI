...
def test_octave_call_ai_v2_compiles_to_effective_managed_pipeline_with_embeddings():
    ...
    assert effective.llm.provider == "octave_call_ai"
    assert effective.llm.model == "default"
    assert effective.tts.provider == "octave_call_ai"
    assert effective.tts.speed == 1.2
    assert effective.stt.provider == "octave_call_ai"
    assert effective.stt.language == "multi"
    assert effective.embeddings.provider == "octave_call_ai"
    assert effective.embeddings.model == "default"
    assert effective.managed_service_version == 2


def test_octave_call_ai_v2_rejects_non_predefined_speed():
...
def test_byok_v2_rejects_octave_call_ai_provider():
    with pytest.raises(ValidationError):
...
def test_masked_octave_call_ai_key_is_preserved_when_saving_same_mode():
    existing = OrganizationAIModelConfigurationV2(
        mode="octave_call_ai",
        octave_call_ai=OctaveCallAIManagedAIModelConfiguration(api_key="mps-real-secret"),
    )
    incoming = OrganizationAIModelConfigurationV2(
        mode="octave_call_ai",
        octave_call_ai=OctaveCallAIManagedAIModelConfiguration(api_key=mask_key("mps-real-secret")),
    )

    merged = merge_ai_model_configuration_v2_secrets(incoming, existing)

    assert merged.octave_call_ai.api_key == "mps-real-secret"
...
def test_legacy_all_octave_call_ai_pipeline_converts_to_octave_call_ai_v2():
    ...
    assert config.mode == "octave_call_ai"
    assert config.octave_call_ai.api_key == "mps-secret"


def test_legacy_mixed_octave_call_ai_pipeline_converts_to_octave_call_ai_v2():
    legacy = EffectiveAIModelConfiguration(
    ...
    assert config.mode == "octave_call_ai"
    assert config.octave_call_ai.api_key == "mps-tts"
    assert config.octave_call_ai.voice == "default"
...
def test_workflow_model_override_migration_removes_v1_override_and_sets_v2():
    ...
    assert v2_override["mode"] == "octave_call_ai"
    assert v2_override["octave_call_ai"]["api_key"] == "mps-workflow"
...
async def test_migrate_model_configuration_v2_initializes_hosted_mps_billing(
    ...
    expected_response = OrganizationAIModelConfigurationResponse(
        configuration={"version": 2, "mode": "octave_call_ai"},
