...
from pipecat.services.octave_call_ai.stt import OctaveCallAISTTService
from pipecat.services.octave_call_ai.tts import OctaveCallAITTSService
...
def test_octave_call_ai_llm_uses_explicit_mps_correlation_id():
    service = OctaveCallAILLMService(
        api_key="mps-secret",
...
@pytest.mark.asyncio
async def test_octave_call_ai_stt_config_uses_explicit_mps_correlation_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "pipecat.services.octave_call_ai.stt.websocket_connect",
        fake_connect,
    )

    service = OctaveCallAISTTService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=16000,
    )
...
@pytest.mark.asyncio
async def test_octave_call_ai_tts_messages_use_explicit_mps_correlation_id(monkeypatch):
    fake_ws = _FakeWebSocket()
    ...
    monkeypatch.setattr(
        "pipecat.services.octave_call_ai.tts.websocket_connect",
        fake_connect,
    )

    service = OctaveCallAITTSService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=24000,
    )
...
@pytest.mark.asyncio
async def test_octave_call_ai_tts_final_for_missing_context_is_ignored():
    service = OctaveCallAITTSService(api_key="mps-secret")
