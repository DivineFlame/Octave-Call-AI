...
OCTAVE_CALL_AI_MCP_INSTRUCTIONS = """\
You build and edit Octave-Call-AI voice-AI workflows by emitting TypeScript that uses the `@octave-call-ai/sdk` package. Workflows are stored as JSON; this server projects them to TypeScript for editing and parses them back on save.
...
1. `search_docs` — use first for keyword or acronym lookup when the user is asking how Octave-Call-AI works or how to configure something.
