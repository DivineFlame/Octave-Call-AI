...
from api.constants import AUTH_PROVIDER, OCTAVE_CALL_AI_MPS_SECRET_KEY, MPS_API_URL
    ...
    async with httpx.AsyncClient() as client:
        ...
        if AUTH_PROVIDER == "local":
            ...
            response = await client.post(
                ...
                json={
                    "name": "Default Octave-Call-AI Model Service Key",
            ...
            if not OCTAVE_CALL_AI_MPS_SECRET_KEY:
                logger.warning(
                    "Warning: OCTAVE_CALL_AI_MPS_SECRET_KEY not set for authenticated mode"
                )
                raise ValidationError("Missing OCTAVE_CALL_AI_MPS_SECRET_KEY in non oss mode")
            ...
            response = await client.post(
                ...
                headers={"X-Secret-Key": OCTAVE_CALL_AI_MPS_SECRET_KEY},
                timeout=10.0,
            )
        ...
        if response.status_code == 200:
            ...
            if service_key:
                ...
                configuration = {
                    "llm": {
                        "provider": ServiceProviders.OCTAVE_CALL_AI.value,
                        "api_key": [service_key],
                        "model": "default",
                    },
                    "tts": {
