"""Quota checking service for Octave-Call-AI credits.
...
from api.services.managed_model_services import (
    ...
    get_octave_call_ai_service_api_key,
    uses_managed_model_services_v2,
)
from api.services.mps_service_key_client import mps_service_key_client

MINIMUM_OCTAVE_CALL_AI_CREDITS_FOR_CALL = 0.10

LEGACY_QUOTA_EXCEEDED_MESSAGE = (
    "You have exhausted your trial credits. "
    "Please email founders@octave-call-ai.com for additional Octave-Call-AI credits "
    "or change providers in Models configurations."
)

BILLING_V2_QUOTA_EXCEEDED_MESSAGE = (
    "You have exhausted your Octave-Call-AI credits. "
    "Please purchase more credits from /billing "
    "or change providers in Models configurations."
)

SERVICE_TOKEN_ORG_MISMATCH_MESSAGE = (
    "The Octave-Call-AI service token being used is created from another account. "
...
def _service_uses_octave_call_ai(service: Any) -> bool:
    provider = getattr(service, "provider", None)
    return (
        provider == ServiceProviders.OCTAVE_CALL_AI or provider == ServiceProviders.OCTAVE_CALL_AI.value
    )


def _octave_call_ai_api_keys(user_config: Any) -> set[str]:
    api_keys: set[str] = set()
    for section_name in ("llm", "stt", "tts", "embeddings"):
        service = getattr(user_config, section_name, None)
        if not _service_uses_octave_call_ai(service):
            continue
...
async def _authorize_hosted_workflow_run_start(
    *,
    ...
    service_key = (
        get_octave_call_ai_service_api_key(user_config) if requires_correlation else None
    )
    ...
    try:
        authorization = await mps_service_key_client.authorize_workflow_run_start(
            ...
            minimum_credits=MINIMUM_OCTAVE_CALL_AI_CREDITS_FOR_CALL,
            created_by=(
            ...
            metadata={
                "octave_call_ai_user_id": str(workflow_owner.id),
        ...
        return (
            QuotaCheckResult(
                ...
                error_message="Could not verify Octave-Call-AI credits. Please try again.",
            ),
            True,
        )
    ...
    if (
        ...
        or remaining < MINIMUM_OCTAVE_CALL_AI_CREDITS_FOR_CALL
    ):
        logger.warning(
            "Insufficient Octave-Call-AI billing v2 credits for org {}: {:.2f} credits remaining",
            organization_id,
            remaining,
        )
    ...
    logger.info(
        "Octave-Call-AI billing v2 run authorization passed for org {}: {:.2f} credits remaining",
        organization_id,
        remaining,
    )
...
async def _authorize_legacy_octave_call_ai_keys(
    *,
    octave_call_ai_api_keys: set[str],
    organization_id: int | None,
    workflow_owner: UserModel,
) -> QuotaCheckResult:
    for api_key in octave_call_ai_api_keys:
        try:
            ...
            if remaining < MINIMUM_OCTAVE_CALL_AI_CREDITS_FOR_CALL:
                logger.warning(
                    f"Insufficient Octave-Call-AI credits for key ...{api_key[-8:]}: "
                    f"${remaining:.2f} remaining"
                )
                return _insufficient_legacy_quota_result()

            logger.info(
                f"Octave-Call-AI quota check passed for key ...{api_key[-8:]}: "
                f"{remaining:.2f} credits remaining"
            )
        except Exception as e:
            logger.error(f"Failed to check quota for Octave-Call-AI key: {str(e)}")
            error_str = str(e)
...
async def _authorize_oss_managed_v2_correlation(
    *,
    ...
    service_key = get_octave_call_ai_service_api_key(user_config)
    if not service_key:
        return QuotaCheckResult(
            has_quota=False,
            error_code="invalid_service_key",
...
async def authorize_workflow_run_start(
    *,
    ...
    try:
        ...
        octave_call_ai_api_keys = _octave_call_ai_api_keys(user_config)
        if not octave_call_ai_api_keys:
            return QuotaCheckResult(has_quota=True)

        legacy_result = await _authorize_legacy_octave_call_ai_keys(
            octave_call_ai_api_keys=octave_call_ai_api_keys,
            organization_id=(
