...
async def create_transport(
    ...
    *,
    ...
    # not Octave-Call-AI's: ``inbound_encoding`` is what we *send into the call*
    # (Octave-Call-AI → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → Octave-Call-AI).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # Octave-Call-AI → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → Octave-Call-AI; from media_format.encoding
