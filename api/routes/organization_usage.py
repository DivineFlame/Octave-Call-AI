...
class CurrentUsageResponse(BaseModel):
    period_start: str
    period_end: str
    used_octave_call_ai_tokens: float
...
class WorkflowRunUsageResponse(BaseModel):
    ...
    name: str
    created_at: str
    octave_call_ai_token_usage: float
...
class UsageHistoryResponse(BaseModel):
    ...
    total_octave_call_ai_tokens: float
    total_duration_seconds: int
    total_count: int
    page: int
    limit: int
...
async def create_mps_credit_purchase_url(
    ...
    """Create a checkout URL for organizations using Octave-Call-AI-managed MPS v2."""
    if DEPLOYMENT_MODE == "oss":
        raise HTTPException(
            status_code=404,
    ...
    try:
        session = await mps_service_key_client.create_credit_purchase_url(
            ...
            billing_details={
                "source": "octave_call_ai_billing",
                "octave_call_ai_user_id": str(user.id),
                "octave_call_ai_provider_id": str(user.provider_id),
            },
        )
    except Exception as exc:
...
async def get_usage_history(
    ...
    try:
        ...
        return {
            "runs": runs,
            "total_octave_call_ai_tokens": total_tokens,
