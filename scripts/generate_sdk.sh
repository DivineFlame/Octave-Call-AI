...
# Requires:
#   - `python` in the `octave_call_ai` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t octave-call-ai-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t octave-call-ai-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m octave_call_ai_sdk.codegen \
    --input "$SPECS_JSON" \
...
datamodel-codegen \
    ...
    --output "sdk/python/src/octave_call_ai_sdk/_generated_models.py" \
...
python -m sdk.codegen.client_codegen \
    --input "$OPENAPI_JSON" \
    --py-out "sdk/python/src/octave_call_ai_sdk/_generated_client.py" \
