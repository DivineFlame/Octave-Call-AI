#!/usr/bin/env bash
# Cut a release of both SDKs — octave-call-ai-sdk (PyPI) and @octave-call-ai/sdk (npm) —
...
#   - `npm login` completed as a member of the `octave_call_ai` npm org. npm
...
if ! NPM_USER="$(npm whoami 2>/dev/null)"; then
    ...
    echo "       octave-call-ai org before re-running this script — otherwise PyPI" >&2
...
if ! git diff --quiet -- sdk/python/src/octave_call_ai_sdk/typed sdk/typescript/src/typed; then
    echo
...
echo
echo "============================================================"
echo "  Built octave-call-ai-sdk==$VERSION and @octave-call-ai/sdk@$VERSION"
echo "  Nothing has been published yet."
echo "============================================================"
echo

if confirm "Upload octave-call-ai-sdk==$VERSION to TestPyPI first (recommended)?"; then
    (cd sdk/python && twine upload --repository testpypi dist/*)
    echo "  → https://test.pypi.org/project/octave-call-ai-sdk/$VERSION/"
    echo
fi

if confirm "Publish @octave-call-ai/sdk@$VERSION to npm? (will prompt for 2FA OTP)"; then
    (cd sdk/typescript && npm publish --access public)
    echo "  → https://www.npmjs.com/package/@octave-call-ai/sdk/v/$VERSION"
    echo
fi

if confirm "Upload octave-call-ai-sdk==$VERSION to PyPI?"; then
    (cd sdk/python && twine upload dist/*)
    echo "  → https://pypi.org/project/octave-call-ai-sdk/$VERSION/"
    echo
fi

if confirm "Create annotated git tag sdks-v$VERSION at HEAD?"; then
    git tag -a "sdks-v$VERSION" -m "octave-call-ai-sdk + @octave-call-ai/sdk $VERSION"
