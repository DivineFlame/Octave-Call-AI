...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              Octave-Call-AI Custom Domain Setup                      ║"
echo "║     Automated Let's Encrypt SSL certificate setup            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

if [[ $EUID -ne 0 ]]; then
    octave_call_ai_fail "This script must be run as root or with sudo"
fi

if [[ ! -d "octave-call-ai" ]]; then
    echo -e "${RED}Error: 'octave-call-ai' directory not found.${NC}"
    echo -e "${YELLOW}Please run this script from the directory containing your Octave-Call-AI installation.${NC}"
    echo -e "${YELLOW}If you haven't set up Octave-Call-AI yet, run the remote setup first:${NC}"
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/DivineFlame/Octave-Call-AI/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || octave_call_ai_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    octave_call_ai_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || octave_call_ai_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    octave_call_ai_warn "Warning: Could not detect server's public IP"
...
if command -v apt-get &> /dev/null; then
    ...
    octave_call_ai_fail "Could not detect package manager. Please install certbot manually."
fi
echo -e "${GREEN}✓ Certbot installed${NC}"

echo -e "${BLUE}[3/7] Stopping Octave-Call-AI services...${NC}"
cd octave-call-ai
...
octave_call_ai_load_env_file .env

if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(octave_call_ai_infer_server_ip "$(pwd)" || true)"
fi

[[ -n "${SERVER_IP:-}" ]] || octave_call_ai_fail "Could not determine SERVER_IP from the existing install"

octave_call_ai_set_env_key .env SERVER_IP "$SERVER_IP"
octave_call_ai_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
octave_call_ai_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
octave_call_ai_delete_env_key .env BACKEND_URL
...
OCTAVE_CALL_AI_PATH="$(pwd)"

cat > /etc/letsencrypt/renewal-hooks/deploy/octave-call-ai-reload.sh << HOOK_EOF
#!/bin/bash
cp /etc/letsencrypt/live/$DOMAIN_NAME/fullchain.pem $OCTAVE_CALL_AI_PATH/certs/local.crt
cp /etc/letsencrypt/live/$DOMAIN_NAME/privkey.pem $OCTAVE_CALL_AI_PATH/certs/local.key
chmod 644 $OCTAVE_CALL_AI_PATH/certs/local.crt $OCTAVE_CALL_AI_PATH/certs/local.key

cd $OCTAVE_CALL_AI_PATH
docker compose --profile remote restart nginx 2>/dev/null || true
HOOK_EOF
chmod +x /etc/letsencrypt/renewal-hooks/deploy/octave-call-ai-reload.sh
...
echo ""
echo -e "${BLUE}[7/7] Starting Octave-Call-AI services through validated startup wrapper...${NC}"
./remote_up.sh
...
echo -e "  Certificate: $OCTAVE_CALL_AI_PATH/certs/local.crt"
echo -e "  Private Key: $OCTAVE_CALL_AI_PATH/certs/local.key"
echo -e "  Auto-renewal: Enabled (certificates renew automatically)"
echo ""
echo -e "${YELLOW}Files modified:${NC}"
echo "  - octave-call-ai/.env (canonical public host/base URL updated)"
echo "  - octave-call-ai/certs/local.crt (SSL certificate)"
echo "  - octave-call-ai/certs/local.key (SSL private key)"
echo "  - /etc/letsencrypt/renewal-hooks/deploy/octave-call-ai-reload.sh (renewal hook)"
echo ""
