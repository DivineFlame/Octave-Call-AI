...
function Download-BundleFileForRef([string]$Destination, [string]$RemotePath, [string]$Ref) {
    $rawBase = "https://raw.githubusercontent.com/DivineFlame/Octave-Call-AI/$Ref"
    $fallbackBase = 'https://raw.githubusercontent.com/DivineFlame/Octave-Call-AI/main'
...
function Download-InitSupportBundle([string]$ProjectDir, [string]$Ref) {
    ...
    Download-BundleFileForRef (Join-Path $ProjectDir 'scripts/run_octave_call_ai_init.sh') 'scripts/run_octave_call_ai_init.sh' $Ref
...
Write-Info ''
Write-Info '╔══════════════════════════════════════════════════════════════╗'
Write-Info '║                    Octave-Call-AI Local Setup                        ║'
...
if ($env:OCTAVE_CALL_AI_SKIP_DOWNLOAD -ne '1') {
    if ($UseCoturn) {
    ...
    Download-File 'https://raw.githubusercontent.com/DivineFlame/Octave-Call-AI/main/docker-compose.yaml' (Join-Path $CurrentDir 'docker-compose.yaml')
    if ($UseCoturn) {
...
if ($UseCoturn) {
    Assert-PathExists 'scripts/run_octave_call_ai_init.sh' 'scripts/run_octave_call_ai_init.sh not found. Re-run setup_local.ps1 without OCTAVE_CALL_AI_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'scripts/lib/setup_common.sh' 'scripts/lib/setup_common.sh not found. Re-run setup_local.ps1 without OCTAVE_CALL_AI_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'deploy/templates/turnserver.remote.conf.template' 'deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.ps1 without OCTAVE_CALL_AI_SKIP_DOWNLOAD=1, or use a full repo checkout.'
}
