...
$UpstreamUrl    = 'https://github.com/DivineFlame/Octave-Call-AI.git'
$CanonicalHttps = $UpstreamUrl
$CanonicalSsh   = 'git@github.com:DivineFlame/Octave-Call-AI.git'
...
Write-Host '|              Octave-Call-AI Contributor Bootstrap                    |' -ForegroundColor Blue
Write-Host '+==============================================================+' -ForegroundColor Blue
Write-Host ''
...
if ($needsForkPrompt) {
    Write-Host 'Enter your fork URL (e.g. https://github.com/<YOUR_HANDLE>/octave-call-ai.git):' -ForegroundColor Yellow
...
if (-not $existingUpstream) {
    ...
    $reset = (Read-Host 'Reset upstream to DivineFlame/Octave-Call-AI? [y/N]').Trim()
    if ($reset -match '^[Yy]') {
