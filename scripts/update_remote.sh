...
REPO="DivineFlame/Octave-Call-AI"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)

echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                  Octave-Call-AI Remote Update                        ║"
echo "║  Refresh deployment files and validate runtime config        ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

[[ -f docker-compose.yaml ]] || octave_call_ai_fail "docker-compose.yaml not found in $(pwd)"
[[ -f .env ]] || octave_call_ai_fail ".env not found in $(pwd)"
...
if [[ -f docker-compose.override.yaml ]]; then
    ...
    echo ""
    echo -e "${YELLOW}See https://github.com/DivineFlame/Octave-Call-AI/tree/main/docs/deployment/update#updating-a-source-build${NC}"
    exit 1
fi
...
[[ -n "${TURN_SECRET:-}" ]] || octave_call_ai_fail "TURN_SECRET not found in .env"
...
if [[ -z "$TARGET_VERSION" ]]; then
    octave_call_ai_info "Fetching latest release tag from GitHub..."
    LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)

    if [[ -z "$LATEST_TAG" ]]; then
        octave_call_ai_warn "Could not auto-discover latest tag - defaulting to 'main'."
        LATEST_TAG="main"
    fi
    ...
    if [[ -t 0 ]]; then
        ...
        echo -e "${YELLOW}full git tag (octave-call-ai-v1.28.0), or 'main' for bleeding edge.${NC}"
...
if [[ "$TARGET_VERSION" == "latest" ]]; then
    TARGET_VERSION=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)
    [[ -n "$TARGET_VERSION" ]] || octave_call_ai_fail "could not resolve 'latest' to a release tag"
fi
...
case "$TARGET_VERSION" in
    ...
    octave-call-ai-*)
        ;;
    v*)
        TRY_TAGS+=("octave-call-ai-$TARGET_VERSION")
        ;;
    *)
        TRY_TAGS+=("octave-call-ai-v$TARGET_VERSION" "v$TARGET_VERSION" "octave-call-ai-$TARGET_VERSION")
        ;;
esac

octave_call_ai_info "Validating target version: $TARGET_VERSION..."
RESOLVED_TAG=""
...
[[ -n "$RESOLVED_TAG" ]] || octave_call_ai_fail "could not find a git tag matching '$TARGET_VERSION'"

if [[ "$RESOLVED_TAG" != "$TARGET_VERSION" ]]; then
    octave_call_ai_success "✓ Resolved '$TARGET_VERSION' to git tag '$RESOLVED_TAG'"
fi
...
case "$TARGET_VERSION" in
    octave-call-ai-v*) IMAGE_TAG="${TARGET_VERSION#octave-call-ai-v}" ;;
...
if [[ -n "$IMAGE_TAG" ]]; then
    if curl -fsI "https://hub.docker.com/v2/repositories/divineflame/octave-call-ai-api/tags/$IMAGE_TAG/" >/dev/null 2>&1; then
        octave_call_ai_success "✓ Image tag :$IMAGE_TAG found on Docker Hub"
    else
        octave_call_ai_warn "Warning: image tag :$IMAGE_TAG not found on Docker Hub - leaving images at :latest"
        IMAGE_TAG=""
    fi
fi
...
echo -e "  Server IP:        ${BLUE}$(octave_call_ai_infer_server_ip "$(pwd)" || echo "unknown")${NC}"
...
if [[ -t 0 && "${OCTAVE_CALL_AI_UPDATE_YES:-}" != "1" ]]; then
...
for f in \
    ...
    .env \
    remote_up.sh \
    scripts/run_octave_call_ai_init.sh \
...
octave_call_ai_download_remote_support_bundle "$(pwd)" "$TARGET_VERSION"
rm -f nginx.conf turnserver.conf

if [[ -n "$IMAGE_TAG" ]]; then
    sed -i.tmp -E "s#(octave-call-ai-(api|ui)):latest#\1:$IMAGE_TAG#g" docker-compose.yaml
    rm -f docker-compose.yaml.tmp
    octave_call_ai_success "✓ docker-compose.yaml updated; images pinned to :$IMAGE_TAG"
else
    octave_call_ai_success "✓ docker-compose.yaml updated (image tags left at :latest)"
fi

echo -e "${BLUE}[3/3] Synchronizing environment and validating init-based remote config...${NC}"
octave_call_ai_set_env_key .env FASTAPI_WORKERS "$FASTAPI_WORKERS"
octave_call_ai_prepare_remote_install "$(pwd)"
docker compose config -q
octave_call_ai_success "✓ Remote init configuration validated"
...
echo ""
echo -e "  ${BLUE}for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_octave_call_ai_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do${NC}"
