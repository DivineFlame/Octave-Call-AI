...
A step-by-step guide to deploying a voice AI stack (Octave-Call-AI + Claude/Gemini + STT/TTS) entirely within a customer's GCP project, with no data leaving their VPC.
...
- A **GKE cluster** running Octave-Call-AI (orchestration + Pipecat pipeline).
...
│  ┌─────────────────────── VPC: octave-call-ai-vpc ─────────────────────────────┐   │
│  │                                                                     │   │
│  │   ┌──────────────────┐         ┌──────────────────────────┐         │   │
│  │   │  GKE Cluster     │  ────▶  │  PSC Endpoint            │ ──▶ Vertex AI
│  │   │  (Octave-Call-AI pods)   │         │  192.168.255.230         │ ──▶ Cloud STT
...
This is the single piece of plumbing that gives Octave-Call-AI private access to Vertex AI (Claude/Gemini), Cloud STT, and Cloud TTS.

### 1.1 Set variables and enable APIs

```bash
export PROJECT_ID=$(gcloud config get-value project)
export NETWORK=octave-call-ai-vpc
...
gcloud compute networks subnets create octave-call-ai-subnet \
  --network=$NETWORK \
  --range=10.0.0.0/20 \
  --region=$REGION \
...
```bash
gcloud compute addresses create octave-call-ai-psc-ip \
  --global \
  --purpose=PRIVATE_SERVICE_CONNECT \
  --addresses=$PSC_IP \
  --network=$NETWORK

gcloud compute forwarding-rules create octave-call-ai-psc-googleapis \
  --global \
  --network=$NETWORK \
  --address=octave-call-ai-psc-ip \
...
## Phase 3 — Deploy Octave-Call-AI on GKE in the same VPC

### 3.1 Create a private GKE cluster

```bash
gcloud container clusters create octave-call-ai-cluster \
  --region=$REGION \
  --network=$NETWORK \
  --subnetwork=octave-call-ai-subnet \
...
gcloud container clusters get-credentials octave-call-ai-cluster --region=$REGION
```

### 3.2 Install Octave-Call-AI via Helm

```bash
helm install octave-call-ai ./charts/octave-call-ai -n octave-call-ai --create-namespace
```

Octave-Call-AI pods inherit the VPC's DNS, so any call to `aiplatform.googleapis.com`, `speech.googleapis.com`, or `texttospeech.googleapis.com` automatically routes through the PSC endpoint.
...
kubectl create serviceaccount octave-call-ai-ksa -n octave-call-ai

# GCP service account
gcloud iam service-accounts create octave-call-ai-gsa

# Grant Vertex AI access
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:octave-call-ai-gsa@$PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/aiplatform.user"

# Bind KSA to GSA
gcloud iam service-accounts add-iam-policy-binding \
  octave-call-ai-gsa@$PROJECT_ID.iam.gserviceaccount.com \
  --member="serviceAccount:$PROJECT_ID.svc.id.goog[octave-call-ai/octave-call-ai-ksa]" \
  --role="roles/iam.workloadIdentityUser"

# Annotate KSA
kubectl annotate serviceaccount octave-call-ai-ksa -n octave-call-ai \
  iam.gke.io/gcp-service-account=octave-call-ai-gsa@$PROJECT_ID.iam.gserviceaccount.com
```
...
gcloud container node-pools create gpu-pool \
  --cluster=octave-call-ai-cluster \
  --region=$REGION \
...
gcloud access-context-manager perimeters create octave-call-ai-perimeter \
  --title="Octave-Call-AI VPC-SC Perimeter" \
...
gcloud kms keyrings create octave-call-ai-keyring --location=$REGION
gcloud kms keys create octave-call-ai-key \
  --keyring=octave-call-ai-keyring \
  --location=$REGION \
  --purpose=encryption
```
...
Run these from a Octave-Call-AI pod inside the cluster:
...
gcloud compute networks subnets create octave-call-ai-subnet --network=$NETWORK \
  --range=10.0.0.0/20 --region=$REGION --enable-private-ip-google-access

# 4. PSC endpoint
gcloud compute addresses create octave-call-ai-psc-ip --global \
  --purpose=PRIVATE_SERVICE_CONNECT --addresses=$PSC_IP --network=$NETWORK
gcloud compute forwarding-rules create octave-call-ai-psc-googleapis --global \
  --network=$NETWORK --address=octave-call-ai-psc-ip --target-google-apis-bundle=all-apis
...
# 6. GKE
gcloud container clusters create octave-call-ai-cluster --region=$REGION \
  --network=$NETWORK --subnetwork=octave-call-ai-subnet \
