// Unit tests for @octave-call-ai/sdk. Uses Node's built-in `node:test` runner and
// an in-memory spec stub — no HTTP, no backend dependency. Mirrors the
// Python SDK tests in api/tests/test_octave_call_ai_sdk.py.
//
...
// ─── OctaveCallAIClient HTTP plumbing (stubbed fetch) ───────────────────────────

describe("OctaveCallAIClient", () => {
    it("sends the API key as X-API-Key", async () => {
        ...
        const c = new OctaveCallAIClient({
