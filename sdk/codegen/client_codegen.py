...
`OctaveCallAIClient` mixes in this class to get HTTP methods for every route
...
from octave_call_ai_sdk._generated_models import (
{imports}
)


class _GeneratedClient:
    # `OctaveCallAIClient.__init__` installs `self._request` (see client.py).
...
// Regenerate with `./scripts/generate_sdk.sh`.
//
// `OctaveCallAIClient` extends this base to get HTTP methods for every route
