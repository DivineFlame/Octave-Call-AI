---
title: "Custom Domain"
description: "Deploy Octave-Call-AI AI with custom domain names and SSL certificates"
---

Deploy Octave-Call-AI AI with your own custom domain name for a professional production setup. By now, you should be able to create and test a voice agent by following the previous guide to setup the platform on a remote server using [Docker](docker#option-2%3A-remote-server-deployment)

## What is Custom Domain Deployment?

Custom domain deployment allows you to run Octave-Call-AI AI with a personalized domain name (like `voice.yourcompany.com`) instead of using IP addresses. This setup includes:
...
- Octave-Call-AI AI already running on your server via the [remote deployment](docker#option-2%3A-remote-server-deployment) guide
...
<Note>
You must be at the same place where you had run `setup_remote.sh` from. The directory should contain `octave-call-ai/` with the artifacts that got created when `setup_remote.sh` was run.
</Note>
<Note>
You must not move the `octave-call-ai/` directory to a different location after running `setup_custom_domain.sh`, since we set up auto certificate renewal script as certbot renewal hook pointing to the `octave-call-ai/` directory.
</Note>

```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/DivineFlame/Octave-Call-AI/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
- Restart Octave-Call-AI services through the validated startup wrapper
...
### Stop Octave-Call-AI Services

Before generating certificates, stop the running Octave-Call-AI services to free up port 80:
...
### Copy Certificates to Octave-Call-AI Directory

Copy the generated certificates to the octave-call-ai certs directory:
...
### Start Octave-Call-AI Services

Start Octave-Call-AI through the validated startup wrapper so `octave-call-ai-init` regenerates nginx and coturn runtime config before Docker starts:
...
```bash
sudo nano /etc/letsencrypt/renewal-hooks/deploy/octave-call-ai-reload.sh
```
...
```bash
#!/bin/bash
# Copy renewed certificates to octave-call-ai certs directory
cp /etc/letsencrypt/live/voice.yourcompany.com/fullchain.pem /home/ubuntu/octave-call-ai/certs/local.crt
cp /etc/letsencrypt/live/voice.yourcompany.com/privkey.pem /home/ubuntu/octave-call-ai/certs/local.key
chmod 644 /home/ubuntu/octave-call-ai/certs/local.crt /home/ubuntu/octave-call-ai/certs/local.key

# Restart nginx to load new certificates
cd /home/ubuntu/octave-call-ai
...
```bash
sudo chmod +x /etc/letsencrypt/renewal-hooks/deploy/octave-call-ai-reload.sh
```
...
1. Verify the certificates were copied correctly: `ls -la octave-call-ai/certs/`
2. Run `./remote_up.sh --preflight-only` in `octave-call-ai/` to verify the `octave-call-ai-init` runtime render matches `.env`
