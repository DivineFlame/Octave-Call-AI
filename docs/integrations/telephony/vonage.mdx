...
description: "Configure Vonage (Nexmo) for voice communication in Octave-Call-AI AI"
---

## Overview

Vonage (formerly Nexmo) is a cloud communications platform that provides global voice, messaging, and video capabilities. Octave-Call-AI AI's Vonage integration enables high-quality voice interactions with your agents using Vonage's robust infrastructure.
...
### Step 3: Configure in Octave-Call-AI AI
...
Vonage configures inbound webhooks at the **application level**, not per phone number. A single **Answer URL** on the Vonage application applies to every number linked to it. Octave-Call-AI routes the call to the right agent based on the called number's inbound workflow assignment inside Octave-Call-AI. **When you save an inbound workflow on a phone number, Octave-Call-AI automatically pushes the webhook URL to your Vonage Application's Answer URL** (provided the credentials are correct).

### Step 1: Link Phone Numbers to Your Vonage Application

1. Open the [Vonage Dashboard](https://dashboard.nexmo.com/)
2. Under **Numbers** → **Your Numbers**, link each number you want to use for inbound to the same Vonage Application whose ID you configured in Octave-Call-AI
...
2. Under **Capabilities** → **Voice**, confirm:
   ...
   <Note>
     ...
     in Step 2. If the field is empty, shows a different URL, or Octave-Call-AI
     surfaced a sync warning on save, the auto-push failed — most often
     because the API Key/Secret or Application ID in Octave-Call-AI is incorrect.
     Paste the URL into the field yourself, set the method to `POST`, and
     save the application. On self-hosted Octave-Call-AI, replace `api.octave-call-ai.com`
...
3. Check call logs in both Octave-Call-AI AI dashboard and Vonage Dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    - Verify the Vonage application's Answer URL is set to `http://localhost:8000/api/v1/telephony/inbound/run`
    - Ensure the Answer URL is publicly accessible
    - Confirm the called number is linked to the correct Vonage application
    - Confirm the called number exists in your Octave-Call-AI telephony configuration and has an **Inbound workflow** assigned
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    - Verify API Key matches the one stored in your Octave-Call-AI telephony configuration (used to identify the org from the inbound webhook)
