---
title: "Agent Stream"
description: "Stream audio to a Octave-Call-AI agent over a WebSocket"
---

## Overview

Agent Stream is a WebSocket endpoint that lets a telephony provider point its media stream at a single URL and drive a Octave-Call-AI agent run. The agent UUID in the URL selects the agent; provider-specific identifiers in the query string (for Cloudonix: `Domain`) tell Octave-Call-AI which stored telephony configuration to use for that call. The bearer token and other credentials are never passed in the URL — they live in the stored configuration and are used by Octave-Call-AI to validate the session and to issue provider API calls (hangup, transfer) during the call.

This is useful when:

- You're integrating Octave-Call-AI into a SIP gateway or in-house dialer that already speaks a supported provider's streaming protocol
...
<Warning>
  ...
  request on [GitHub Discussions](https://github.com/DivineFlame/Octave-Call-AI/discussions)
  with your use case.
</Warning>
...
```
wss://app.octave-call-ai.com/api/v1/agent-stream/{agent_uuid}
```

`{agent_uuid}` is the agent's stable UUID (see [Get the Agent UUID](#get-the-agent-uuid) below). On self-hosted deployments, replace `app.octave-call-ai.com` with your backend host.

## Prerequisites

- A Octave-Call-AI agent (workflow) — published or in draft is fine
- A Cloudonix telephony configuration in your Octave-Call-AI organization whose `domain_id` matches the `Domain` you pass on the URL. Octave-Call-AI uses the bearer token from this configuration to validate the call session and to issue provider API calls (hangup, transfer).
...
| `Domain` | Yes (cloudonix) | Cloudonix domain ID. Octave-Call-AI uses this to look up the matching stored telephony configuration and retrieve the bearer token used for provider API calls. |
...
Use this URL inside the CXML `<Stream>` your Cloudonix Voice Application returns when the call needs to be bridged to the Octave-Call-AI agent:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
    <Stream url="wss://app.octave-call-ai.com/api/v1/agent-stream/{agent_uuid}?provider=cloudonix&Domain=...&callId=...&from=...&to=..."/>
  </Connect>
  <Pause length="40"/>
</Response>
```

The first two messages on the socket should be Cloudonix's standard `connected` and `start` events (Twilio-compatible framing). Octave-Call-AI extracts `streamSid` and `callSid` from the `start` event payload, validates the session against Cloudonix using the bearer token from the stored telephony configuration matched by `Domain`, and then begins streaming audio.

## Workflow run lifecycle

When the WebSocket is accepted, Octave-Call-AI:
...
- No bearer tokens or provider secrets are passed in the URL. Provider credentials live in the stored telephony configuration (matched by `Domain` for Cloudonix) and are used server-side by Octave-Call-AI to validate the session and issue provider API calls.
