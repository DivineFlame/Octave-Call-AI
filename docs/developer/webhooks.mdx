...
description: "Context variables available in webhook nodes and the data Octave-Call-AI sends after a call"
---

Octave-Call-AI executes **webhook nodes** asynchronously after a workflow run completes. You configure the target URL, HTTP method, headers, and payload template directly in the workflow definition.
...
2. Octave-Call-AI executes any `webhook` nodes in the workflow asynchronously
...
@app.post("/webhook/octave-call-ai")
async def handle_octave_call_ai_webhook(request: Request):
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "http_method": "POST",
    "endpoint_url": "https://your-service.com/webhook/octave-call-ai",
    "payload_template": {
      "run_id": "{{workflow_run_id}}",
      "customer": "{{initial_context.customer_name}}",
      "outcome": "{{gathered_context.resolution}}"
