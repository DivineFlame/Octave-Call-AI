...
sequenceDiagram
    ...
    participant API as Octave-Call-AI API
...
**Telephony**
The phone infrastructure. Octave-Call-AI connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and Octave-Call-AI in real time.

**Transcriber (STT)**
Converts the caller's speech to text in real time. Octave-Call-AI sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. Octave-Call-AI instructs your telephony provider to dial the number
...
6. When an edge condition is met, Octave-Call-AI transitions to the next node
